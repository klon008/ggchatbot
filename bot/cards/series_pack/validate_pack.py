...
def validate_bot_root(bot_root: Path) -> list[str]:
    errs: list[str] = []
...
def validate_frontend_root(fe_root: Path) -> list[str]:
    errs: list[str] = []
...
def validate_pack(
    ...
    mig_text = "\n".join(
        p.read_text(encoding="utf-8", errors="ignore") for p in mig_dir.glob("m*.py")
    )

    if re.search(
        rf'["\']id["\']:\s*["\']{re.escape(sid)}["\']|_SERIES_ID\s*=\s*["\']{re.escape(sid)}["\']',
        mig_text,
    ):
    ...
    # Существующую рубашку переиспользуем (в т.ч. дефолтную card-back.svg)
    if (bot_assets / f"{back_id}.svg").exists():
        pass

    if fe_root is not None:
        fe_imports = fe_root / "src" / "imports"
        catalog_path = fe_root / "src" / "lib" / "cardCatalog.ts"
        catalog = (
            catalog_path.read_text(encoding="utf-8") if catalog_path.is_file() else ""
        )
        for cid in seen:
            if catalog and re.search(rf'["\']{re.escape(cid)}["\']', catalog):
        ...
        # FE рубашку тоже можно переиспользовать — не ошибка
