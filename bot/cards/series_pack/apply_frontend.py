...
def apply_frontend(pack: PackData, fe_root: Path, log: LogFn) -> None:
    ...
    dest_back = imports_dir / f"{back_id}.svg"
    if dest_back.exists():
        log(f"FE рубашка уже есть, переиспользуем: {back_id}.svg")
    else:
...
def _patch_card_backs(fe_root: Path, back_id: str, log: LogFn) -> None:
    ...
    prev_imp = [
        ln for ln in body(M_BACK_IMP_S, M_BACK_IMP_E).splitlines() if back_id not in ln
    ]
    prev_map = [
        ln for ln in body(M_BACK_MAP_S, M_BACK_MAP_E).splitlines() if back_id not in ln
    ]
