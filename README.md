...
Чат-бот для стриминга: зрители заказывают музыку командой `!sr <ссылка YouTube или Яндекс Музыки>`,
...
|---------|-----|----------|
| `!заказ` / `!зм` / `!sr <ссылка>` | все | Заказать трек с YouTube или Яндекс Музыки. |
...
- **Заказ не играет, в консоли `iframe_api ERR_CONNECTION_RESET`** — YouTube заблокирован
  ...
  Пока YouTube API недоступен, YouTube-треки снимаются с рефандом; заказы Яндекс Музыки работают.
...
botmsc/
  ...
  bot/
    ...
    goodgame/             # общий WS-клиент чата GoodGame v2
      client.py
    song_request/         # заказ музыки через YouTube / ЯМузыку + OBS
      handler.py          # команды !sr / !skip / очередь / watchdog
      queue.py            # очередь + атомарная персистентность
      youtube.py          # валидация YouTube
      yandex.py           # валидация Яндекс Музыки
      yandex_stream.py    # скачивание трека (Plus-токен) в data/ym_cache
      validate.py         # роутер провайдера по URL
  ...
  obs/
    player.html
    player.js             # YouTube + <audio> ЯМузыка + WS
  docs/
