...
│   │
│   ├── daycycle/                # анонс смены суток (хуки fishing/steal)
│   │   ├── __init__.py
│   │   └── handler.py           # DaycycleHandler, midnight tick
│   │
...
1. День недели: среда или пятница (MSK) или ручной override из админки.
2. Минимум 5 зрителей в `_viewers`.
3. Кулдаун 10 мин на пользователя; попытка +1, `last_steal_day_key` = сегодня.
4. Рост шанса: каждые 5 попыток +1% (пол 5%, кап 35%). Пропуск ср/пт без `!кража` → −2%.
5. Бросок `1..100` против `chance`.
6. ~1% — джекпот из казны (сумма тира ×2); иначе случайная жертва из чата.
7. Жертва ≥3000 на руках, без сейфа; сумма — взвешенный тир (мелочь/норма/жир/куш), жертве остаётся ≥3000.
8. Тюрьма только после реального списания; при посадке — полный возврат жертве/в казну.

Анонс «день кражи» / «день кражи закончился» по расписанию — через `bot/daycycle` (вместе с анонсом смены суток рыбалки). Ручные open/close в админке — мгновенные отдельные сообщения.

Админка «Кража»: таблица `steal_stats`, редактор весов тиров лута.
...
| `apply_attempt_growth(info)` | +1% шанса каждые 5 попыток, кап 35% |
| `apply_missed_day_decay(info, day_key)` | −2% за пропуск ср/пт, пол 5% |
| `roll_steal_amount(tiers)` | Взвешенный тир добычи (мелочь/норма/жир/куш) |
| `get_daily_bonus(day_number)` | Бонус N-го использования `!дейлик` в месяце |
| `is_steal_allowed()` | `weekday in (2, 4)` — среда, пятница |
| `prison_chance_for_amount(stolen)` | Шанс тюрьмы по сумме кражи (5–35%) |
