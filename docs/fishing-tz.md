...
|---------|--------------|--------|-------|
| Щит | `!рыбалка защита [N]` (500×N) или редкий дроп с червей / `grant_mermaid_shields` | 1 заряд = одна русалка без штрафа | конец дня |
| Активатор клёва | редкий дроп с червей / `grant_bite_boost` | +10 забросов: miss и trash ÷3 | конец дня |
| Карманный сейф | 1% при `!рыбалка черви` **только пока `!кража` активна** / `grant_steal_safe` | жертву нельзя ограбить до конца суток | конец дня |
