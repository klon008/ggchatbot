...
При смене суток (`day_changed`) к `BAIT_SPOILED` дописывается пул `EVENT_BOOST_DAY` с `{N}`.
...
- «Новый день: наживка протухла, щиты и активатор клёва сгорели…»
- (см. пул `BAIT_SPOILED` в `texts.py`)
- если сегодня рыбный день по расписанию — дописка из `EVENT_BOOST_DAY`
