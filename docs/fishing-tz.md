...
**Есть:** ручной заброс; энергия; черви и опарыш; удочка; щит от русалки; активатор клёва; карманный сейф (дни !кража); продажа; первая рыба дня; недельный топ по весу видов; ручная выдача наград недели + CLI-напоминание; негативные события; сходы; пулы атмосферных текстов; `!рыбалка помощь`; публичный `grants` API.
...
|-----|--------------|--------|
| Черви | `!рыбалка черви` за энергию | −15 энергии → **+5 червей** (редко: щит или активатор клёва вместо червей) |
...
**Сутки:** оба типа → **0** (протухли). Удочка не сбрасывается. Щиты и буст клёва тоже → **0**.
...
### 3.3a. Щит от русалки, активатор клёва, карманный сейф

| Предмет | Как получить | Эффект | Сброс |
|---------|--------------|--------|-------|
| Щит | `!рыбалка защита [N]` (500×N) или редкий дроп с червей / `grant_mermaid_shields` / вкладка «Ивенты» | 1 заряд = одна русалка без штрафа | конец дня |
| Активатор клёва | редкий дроп с червей / `grant_bite_boost` / вкладка «Ивенты» / **рыбный день** (расписание) | +N забросов: miss и trash ÷3 | конец дня |
| Карманный сейф | 1% при `!рыбалка черви` **только пока `!кража` активна** / `grant_steal_safe` / вкладка «Ивенты» | жертву нельзя ограбить до конца суток | конец дня |

Порядок дропа с червей: щит 3% → активатор 3% → сейф 1% (если кража открыта) → иначе черви.

Публичный API: `bot.fishing.grant_mermaid_shields`, `grant_bite_boost`, `grant_steal_safe`, `has_steal_safe` (без чата).

### 3.3b. Рыбный день (расписание буста) и админ-выдача

Конфиг в `fishing_meta.events_json` (вкладка **«Ивенты»**):

- `boost_enabled` (дефолт `true`);
- `boost_weekdays` — дни недели МСК, `datetime.weekday()` (дефолт `[3]` = четверг);
- `boost_casts` — сколько зарядов начислить (дефолт `30`).

При первой любой команде `!рыбалка*` в активный день: если `event_boost_day_key != today`, тихо `bite_boost_casts_left += boost_casts`. Копание буста не отключается. В 00:00 МСК заряды сгорают вместе с дневным ресетом (на пятницу не переносятся).

Анонс смены суток (в т.ч. `BAIT_SPOILED` + при активном расписании `EVENT_BOOST_DAY`) — через модуль `bot/daycycle` (отдельное сообщение в чат), не prefix к ответу `!рыбалка`.

Ручная выдача (щит / буст / сейф) — `POST /api/events/grant`, лог в `fishing_grant_log` (авто-четверг в лог не пишется).

### 3.4. Принцессы (связь с модулем)

**Стоки:** опарыш, удочка, щит от русалки, русалка и аналоги.
...
| `!рыбалка черви` | −15 энергии → +5 червей (или редко щит / активатор / сейф при активной !кража) |
...
| `!рыбалка защита [N]` | Купить N щитов от русалки (500×N, макс. 20) |
| `!рыбалка улов` | Личные рекорды веса по видам + запас ресурсов |
| `!рыбалка топрыба` | Недельный лидерборд |
| `!рыбалка энергия` | Энергия, наживка, удочка, щиты, буст клёва |

Админ (отдельно от публичных подкоманд): восстановление энергии всем; **выдача наград закрытой недели** (см. §10.2, §11.16); вкладка **«Ивенты»** — расписание рыбного дня и ручная выдача щита/буста/сейфа.
...
|-----|-----------|------------------|------------|---------------------|
| Карась | 18 | 0.15–0.80 | 28 | 14 / 28 / 56 |
| Плотва | 14 | 0.10–0.55 | 34 | 17 / 34 / 68 |
| Окунь | 11 | 0.20–1.20 | 56 | 28 / 56 / 112 |
| Линь | 9 | 0.40–3.00 | 72 | 36 / 72 / 144 |
| Язь | 8 | 0.30–2.80 | 100 | 50 / 100 / 200 |
| Лещ | 8 | 0.40–2.00 | 84 | 42 / 84 / 168 |
| Сазан | 7 | 0.80–8.00 | 120 | 60 / 120 / 240 |
| Жерех | 6 | 0.50–4.00 | 130 | 65 / 130 / 260 |
| Судак | 6 | 0.50–5.00 | 140 | 70 / 140 / 280 |
| Щука | 5 | 0.80–4.00 | 126 | 63 / 126 / 252 |
| Сом | 4 | 1.50–8.00 | 196 | 98 / 196 / 392 |
| Осётр | 4 | 2.00–12.00 | 350 | 175 / 350 / 700 |

Сумма весов = 100. Дорогие виды реже; осётр — топ ценности. Цены ≈×2.8 к прежним (EV заброса около нуля при русалке/удочке без изменения).

TODO (следующая волна видов): голавль, амур (белый), налим.
...
| Событие | Шанс | Эффект |
|---------|------|--------|
| Русалка | 1% | −3000 принцесс (не ниже 0); если есть щит — щит −1, штраф 0 |
...
- щиты от русалки, буст клёва и карманный сейф → 0;
- `event_boost_day_key` → '' (можно снова получить ивент-буст в следующий рыбный день);
- флаг «первая рыба дня» сбрасывается;
- удочка без изменений.

**Анонс в чат** — модуль `bot/daycycle` (первое сообщение суток или tick у 00:00 МСК). Хуки собирают одно сообщение:

- fishing: пул `BAIT_SPOILED`; если сегодня рыбный день — ещё `EVENT_BOOST_DAY` с `{N}`;
- princess/steal: при ср/пт — «Сегодня день кражи…»; при уходе с дня кражи — «День кражи закончился…».

Ручные open/close кражи в админке по-прежнему шлют отдельные мгновенные анонсы.
...
В том же JSON — флаги **enabled** по видам (чекбоксы в админке). Выключенный вид **не** выпадает в забросе; при выплате закрытой недели топ по нему **всё равно** оплачивается, если рекорд уже был зафиксирован. Без ключа `enabled` в БД все виды считаются включёнными.
...
Дополнительно **«Рыба недели»** (макс. вес среди всех видов): default **+1200** поверх награды вида (если уже топ вида — суммируется).

Ориентир: весь «джекпот» недели (все виды + FoW) ≈ **8.2k**, не уровень бустера карт (~15k).
...
После любого исхода `!рыбалка` (рыба, сход, мусор, негатив) и в ответах покупок/копания — компактная строка:

```
Энергия: {E}/100 | Наживка: {W+M} | Удочка: целая
```

Наживка = черви + опарыш. Щиты / буст клёва / сейф дописываются **только если активны** (нули и «Сейф: нет» не показывают).

Пример с баффами:

```
Энергия: 31/100 | Наживка: 4 | Удочка: целая | Щиты: 1 | Буст: 10 | Сейф
```

### 11.12. `!рыбалка улов` и `!рыбалка энергия`

**`!рыбалка энергия` (полная карточка):**

- «Энергия: {E}/100 | Черви: {W} | Опарыш: {M} | Удочка: целая | Щиты: {S} | Буст клёва: {B} | Сейф: {SAFE}.»
- «Ресурсы: энергия {E}/100, черви {W}, опарыш {M}. Удочка: {rod_hint}. Щиты: {S}. Буст: {B}. Сейф: {SAFE}.»

**`!рыбалка улов` (рекорды + компактные ресурсы):**

- «Твои рекорды: Карась — 0.78 кг, Окунь — 1.12 кг, Щука — 3.45 кг. Энергия: 90/100 | Наживка: 5 | Удочка: целая.»
- «Пока без рекордов по видам. Запасы: Энергия: {E}/100 | Наживка: {W+M} | Удочка: ….»
