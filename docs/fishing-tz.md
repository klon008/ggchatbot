...
Быстрая чат-мини-игра: `!рыбалка` → мгновенный результат (улов / мусор / событие) → при рыбе сразу продажа за принцессы.

**Есть:** ручной заброс; энергия; черви и опарыш; удочка; продажа; первая рыба дня; недельный топ по весу видов; ручная выдача наград недели + CLI-напоминание; негативные события; пулы атмосферных текстов; `!рыбалка помощь`.
...
7. Каждый ответ на `!рыбалка` (успех, мусор, событие) **самодостаточен**: в конце — строка ресурсов (§11.10).
...
| Опарыш | `!рыбалка опарыш` за принцессы | **−50 принцесс → +10 опарышей** |
...
| Команда | Действие |
|---------|----------|
| `!рыбалка` | Заброс (−5 энергии, −1 наживка) → результат |
| `!рыбалка черви` | −15 энергии → +5 червей |
| `!рыбалка опарыш` | −50 принцесс → +10 опарышей |
...
Админ (отдельно от публичных подкоманд): восстановление энергии всем; **выдача наград закрытой недели** (см. §10.2, §11.15).
...
flowchart TD
  ...
  checks -->|ok| pay["Списать 5 энергии и 1 наживку"]
  pay --> rollEvent["Ролл NegEvent"]
  rollEvent -->|hit| applyNeg["Применить событие и выйти"]
  rollEvent -->|miss| rollCat["Ролл категория: мусор или рыба"]
...
2. Иначе **категория:** мусор **18%**, рыба **82%**.
3. Если рыба — выбор вида по весам §7; вес равномерно в `[w_min, w_max]`; размер и продажа по §6.1.
4. Если это первая **рыба** (не мусор/не событие) в текущих сутках канала — **+500 принцесс** поверх продажи («Первая рыба дня»), один раз на весь канал.

Итог начисления принцесс за успешную рыбу:

```
итог = продажа + (500 если первая_рыба_дня иначе 0)
```
...
|-----|-----------|------------------|------------|---------------------|
| Карась | 28 | 0.15–0.80 | 200 | 100 / 200 / 400 |
| Плотва | 22 | 0.10–0.55 | 250 | 125 / 250 / 500 |
| Окунь | 18 | 0.20–1.20 | 400 | 200 / 400 / 800 |
| Лещ | 12 | 0.40–2.00 | 600 | 300 / 600 / 1200 |
| Щука | 10 | 0.80–4.00 | 900 | 450 / 900 / 1800 |
| Сом | 6 | 1.50–8.00 | 1400 | 700 / 1400 / 2800 |
| Осётр | 4 | 2.00–12.00 | 2500 | 1250 / 2500 / 5000 |

Сумма весов = 100. Дорогие виды реже; осётр — топ ценности.

---

## 8. Мусор (юмор, продажа 0)

Среди исхода «мусор» равновероятно один из шести типов. Продажа = 0. Полные тексты — §11.7.
...
Ролл **до** мусора/рыбы. События взаимоисключающие. Полные тексты — §11.6.
...
Дополнительно **«Рыба недели»** (макс. вес среди всех видов): **+2000** поверх награды вида (если уже топ вида — суммируется).
...
### 11.8. Успешная рыба и «Первая рыба дня»
...
### 11.10. Строка ресурсов (хвост ответа)

После любого исхода `!рыбалка` (рыба, мусор, негатив) добавлять одну строку:
...
### 11.11. `!рыбалка улов` и `!рыбалка энергия`
...
- «Кратко: удочка → наживка → заброс. Черви за энергию, опарыш за 50 принцесс. Первая рыба дня на канале даёт +500. Подкоманды: !рыбалка помощь.»

### 11.14. Админ: восстановление энергии
...
### 11.15. Админ: выдача наград недели
...
### 11.16. Доп. вариации заброса (вступление до результата)
...
- Заброс: −5 энергии, −1 наживка.
- Червяной путь: 15 энергии → 5 забросов по наживке (энергия на забросы ещё −25) ≈ 8 забросов на полный запас без опарыша, если только копать+ловить.
- Опарыш: 5 принцесс/шт. номинально — sink в экономику, экономит энергию копания.
