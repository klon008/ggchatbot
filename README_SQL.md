...
Бот **не** делает auto-migrate JSON → SQLite при старте. Если в `data/` лежат старые JSON — сначала запустите миграционный скрипт (см. [§8](#8-миграция-с-json)).

**Изменения схемы SQLite** (новые таблицы, колонки) применяются **автоматически** при `Database.open()` — при старте бота и при `scripts/migrate_db.py` (вызывается из `update.cmd`). Перед применением создаётся бэкап `data/backup-YYYYMMDD-HHMMSS/bot.db`. Миграции лежат в `bot/db/migrations/`.
...
Версия схемы: **4** (`schema_version`). Миграции: `bot/db/migrations/m00N_*.py`.
...
Ник обновляется при первом сообщении в чате, кнопкой «Получить ники» в админке (онлайн из GG) или вручную при добавлении в points; в админке список points показывает `user_id`, `user_name`, `balance` (LEFT JOIN).
